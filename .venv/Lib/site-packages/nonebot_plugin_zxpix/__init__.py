...
zxpix_menu = on_alconna('.zxpix.menu', aliases={'.zxpix.help'}, priority=5, block=True)

@zxpix_menu.handle()
async def menu():
    message = """======Z-X-P-I-X=====
<1> .zxpix.get [u | p] [~] [tags] [-n num| -r | -noai]
    ==> 随机 | 依据参数获取图片
<2> .zxpix.info ==> 获取当前图库信息
<3> .zxpix.add [u | p] [~] ==> 申请将图片加入图库
<4> .zxpix.stars ==> 查看个人收藏
<5> .zxpix.rank [n=10] [-r] ==> 查看收藏排行
<6> (~回复图片) .original ==> 获取原图
<7> (~回复图片) .star | .unstar | .info
    ==> 收藏 | 取消收藏 | 展示图片信息
<8> (~回复图片) .block 
    ==> 申请屏蔽该图, 可用[~u]参数申请屏蔽该uid所有图片
    """
    await zxpix_menu.finish(message)
