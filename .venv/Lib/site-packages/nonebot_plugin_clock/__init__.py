...
del_clock_qq = on_command('clock.del', aliases={'clock.rm', 'clock.delete', 'clock.remove'}, block=True)
add_clock_qq = on_command('clock.add', aliases={'clock.set'}, block=True)
check_clock_qq = on_command('clock.check', aliases={'clock.ensure'}, block=True)
help_menu = on_command('clock.menu', aliases={'clock.help'}, block=True)

@help_menu.handle()
async def _(matcher: Matcher, event: MessageEvent):
    await matcher.finish(message="""==========<R-I-F-T>==========
1> 使用[.clock.add] 来添加闹钟吧!
==> 格式: .clock.add 8:30 起床咯
==> 或者: .clock.add +30M 衣服洗好了!快去收！
2> 使用[.clock.del] 删除你不需要的闹钟~
==> 格式: .clock.del 3
3> 使用[.clock.check] 查看已经设置的闹钟 ^u^
==> 格式: .clock.check
==========<R-I-F-T>==========
""")
...
def create_clock_scheduler(clock):
    ...
    async def add_clock():
        if clock.verify_today():
            await get_bot().send_msg(message_type=clock.type, user_id=clock.user, group_id=clock.user,
...
async def _(matcher: Matcher, bot: Bot, event: MessageEvent, state: T_State, messages: Message = CommandArg()):
    ...
    if len(messages) < 2:
        await matcher.finish(message="添加格式为: “.clock.add [HH:MM \ +TIME] [内容]”")

    time_ = get_time(messages[0])
    if not time_:
        await matcher.finish(message="时间格式不对哦!")
...
async def _(matcher: Matcher, event: MessageEvent):
    ...
    if clock_msg:
        ...
        await matcher.finish(message='当前没有闹钟哦~\n使用[.clock.add]添加吧!')
...
async def _(matcher: Matcher, event: MessageEvent, ids=CommandArg()):
    ...
    content = ""
    if task[1] is None:
        content = f"成功删除闹钟[{task[0]}]!"
    else:
        content = f"闹钟[{task[1]}]不存在!"
