...
5> .steam.check 查看好友状态
6> .steam.enable | disable 开关steam群播报
7> .steam.update [name] [pic] 更新群名称和群头像
...
async def bind_handle(
    ...
    if user_data := bind_data.get(parent_id, event.get_user_id()):
        ...
        await bind.finish(f"已更新你的steam_id为[{steam_id}]。啊嘞？")
    else:
        bind_data.add(
            parent_id,
...
async def info_handle(
    ...
    if len(at) != 0:
        ...
        if user_data is None:
            await info.finish("该用户未绑定steam_id，找不到哦~")
        ...
        if user_data is None:
            await info.finish(
                "未绑定steam_id, 请使用: .steambind 进行绑定后重试......"
            )
