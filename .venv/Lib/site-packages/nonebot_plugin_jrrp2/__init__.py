...
async def jrrp_handle(bot: Bot, event: Event):
    ...
    await jrrp.finish(Message(
        f'[CQ:at,qq={event.get_user_id()}] 您今日的幸运指数是{lucknum}，为"{luck_simple(lucknum)[0]}"。{luck_simple(lucknum)[1]}'))


alljrrp = on_command("alljrrp", aliases={'总人品', '平均人品', '平均运势'})
...
async def alljrrp_handle(bot: Bot, event: Event):
    ...
    await jrrp.finish(Message(
        f'[CQ:at,qq={event.get_user_id()}] 您一共使用了{times}天jrrp，您历史平均的幸运指数是{round(allnum / times, 1)}'))
...
async def monthjrrp_handle(bot: Bot, event: Event):
    ...
    await jrrp.finish(Message(
        f'[CQ:at,qq={event.get_user_id()}] 您本月共使用了{times}天jrrp，平均的幸运指数是{round(allnum / times, 1)}'))
...
async def weekjrrp_handle(bot: Bot, event: Event):
    ...
    await jrrp.finish(Message(
        f'[CQ:at,qq={event.get_user_id()}] 您本周共使用了{times}次jrrp，平均的幸运指数是{round(allnum / times, 1)}'))
