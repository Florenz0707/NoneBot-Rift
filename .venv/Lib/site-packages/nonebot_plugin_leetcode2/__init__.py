...
request_today = on_command("lc.today", aliases={"lc", "leetcode"}, priority=10, block=True)

request_search = on_command("lc.find", aliases={"lc.search", "leetcode.find", "leetcode.search"}, priority=10,
                            block=True)

request_random = on_command("lc.rand", aliases={"lc.want", "lc.random", "leetcode.rand", "leetcode.want", "leetcode.random"}, priority=10, block=True)

request_user = on_command("lc.care", aliases={"lc.user", "lc.盒", "leetcode.care", "leetcode.user", "leetcode.盒"},
                          priority=10, block=True)

lc_menu = on_command("lc.help", aliases={"lc.menu"}, priority=5, block=True)
...
@lc_menu.handle()
async def menu():
    message = """======Leetcode2==Menu======
[1]: .lc | .lc.today => 每日一题
[3]: .lc.rand | .lc.want => 随机一题
[2]: .lc.find | .lc.search => 搜索题目
[4]: .lc.care | .lc.user | .lc.盒 => 查询用户
===========================
"""
    await lc_menu.send(message)
...
async def send_today_problem(bot: Bot, event: Event):
    try:
        ...
        logger.info(f"[LC查询] 获取每日一题成功，题目标题：{today_title}.")
        today_data = get_sub_problem_data(today_title)
        logger.info("[LC查询] 获取题目内容成功。")
        logger.info(f"today_data[:2] :{today_data[:2]}")
...
async def send_random_problem(bot: Bot, event: Event):
    try:
        ...
        logger.info(f"[LC查询] 获取随机题目成功，题目为{random_title}.")
    ...
    await request_random.send(
        "成功获取到了随机题目~请努力哦!\n" + "\n".join(random_data[:2]) + MessageSegment.image(
            pic) + f"\n{random_data[3]}")
...
async def send_user_data(bot: Bot, event: Event, state: T_State):
    ...
    try:
        ...
        totalSubmissions = user_public_profile["data"]["userProfilePublicProfile"]["submissionProgress"][
            "totalSubmissions"]
        ...
        otherSubmissions = user_public_profile["data"]["userProfilePublicProfile"]["submissionProgress"][
            "otherSubmissions"]
        ...
        numAcceptedQuestions_easy = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numAcceptedQuestions"][0]["count"]
        numAcceptedQuestions_medium = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numAcceptedQuestions"][1]["count"]
        numAcceptedQuestions_difficulty = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numAcceptedQuestions"][2]["count"]
        numFailedQuestions_easy = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numFailedQuestions"][0]["count"]
        numFailedQuestions_medium = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numFailedQuestions"][1]["count"]
        numFailedQuestions_difficulty = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numFailedQuestions"][2]["count"]
        numUntouchedQuestions_easy = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numUntouchedQuestions"][0]["count"]
        numUntouchedQuestions_medium = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numUntouchedQuestions"][1]["count"]
        numUntouchedQuestions_difficulty = \
            user_question_progress["data"]["userProfileUserQuestionProgress"]["numUntouchedQuestions"][2]["count"]
        ...
        acSubmissionNum_easy = \
            user_question_submitstats["data"]["userProfileUserQuestionSubmitStats"]["acSubmissionNum"][0]["count"]
        acSubmissionNum_medium = \
            user_question_submitstats["data"]["userProfileUserQuestionSubmitStats"]["acSubmissionNum"][1]["count"]
        acSubmissionNum_hard = \
            user_question_submitstats["data"]["userProfileUserQuestionSubmitStats"]["acSubmissionNum"][2]["count"]
    ...
    await request_user.send(
        ...
        f"用户名：{realName}(id:{userSlug})\n" +
        f"【已解决问题】  \nEASY：{numAcceptedQuestions_easy} \nMEDIUM：{numAcceptedQuestions_medium} \nDIFFICULTY：{numAcceptedQuestions_difficulty}\n" +
        f"【未答出问题】  \nEASY：{numFailedQuestions_easy} \nMEDIUM：{numFailedQuestions_medium} \nDIFFICULTY：{numFailedQuestions_difficulty}\n" +
        f"【未接触问题】  \nEASY：{numUntouchedQuestions_easy} \nMEDIUM：{numUntouchedQuestions_medium} \nDIFFICULTY：{numUntouchedQuestions_difficulty}\n" +
        f"【历史提交】  \n提交总数：{totalSubmissions} \n错误提交数：{waSubmissions} \n正确提交数：{acSubmissions} \n重新提交数：{reSubmissions} \n其他提交数：{otherSubmissions} \nAC总数：{acTotal}\n" +
        f"【提交通过数】  \nEASY：{acSubmissionNum_easy} \nMEDIUM：{acSubmissionNum_medium} \nDIFFICULTY：{acSubmissionNum_hard}\n" +
        f"【成就贡献】  \n阅读总数：{viewCount} \n获得点赞：{voteCount} \n获得收藏：{favoriteCount} \n发布题解：{columnsUserSolutionCount}\n" +
...
async def send_leetcode_everyday():
    ...
    try:
        today_title = get_today_title()
        logger.info(f"[LC查询] 获取每日一题成功，题目为{today_title}.")
        today_data = get_sub_problem_data(today_title)
        logger.info("[LC查询] 获取题目内容成功。")
        logger.debug(f"[LC查询] 题目{today_data[0]}的难度为{today_data[1]}")
    except Exception as e:
    ...
    try:
        for qq in qq_list:
            await get_bot().call_api("send_private_msg", user_id=qq, message="今日份的题目来咯~加油哦ww\n" + "\n".join(
                today_data[:2]) + MessageSegment.image(pic) + f"\n{today_data[3]}")
        for group in group_list:
            await get_bot().call_api("send_group_msg", group_id=group,
                                     message="今日份的题目来咯~\n今天也不能懈怠!˶╹ꇴ╹˶ \n" + "\n".join(
                                         today_data[:2]) + MessageSegment.image(pic) + f"\n{today_data[3]}")
    except TypeError:
...
try:
    for index, time in enumerate(time_list):
        scheduler.add_job(send_leetcode_everyday, "cron", hour=time["HOUR"], minute=time["MINUTE"],
                          id=f"leetcode_{str(index)}")
